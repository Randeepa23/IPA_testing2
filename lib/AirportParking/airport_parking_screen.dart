...
class _AirportParkingScreenState extends State<AirportParkingScreen> {
  ...
  Future<void> _fetchCheckedInVehicles() async {
    ...
    final vehicles = result.status
        ? await Future.wait(result.vehicles.map((vehicle) async {
            if (vehicle.scheduledCheckOutDate.trim().isNotEmpty) {
              return vehicle;
            }

            final booking =
                await AirportParkingService.fetchBooking(vehicle.referenceNumber);
            final endDate = booking.data?['end_date']?.toString().trim() ?? '';
            return vehicle.withScheduledCheckOutDate(endDate);
          }))
        : <CheckedInVehicle>[];
    if (!mounted) return;
    setState(() {
  ...
  Widget _buildCheckedInVehicleTile(CheckedInVehicle vehicle) {
    ...
    final extraServices = vehicle.extraServices
        .split(',')
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          _formatDateTime(vehicle.scheduledCheckOutDate),
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                            fontSize: 11,
                  ...
                  if (extraServices.isNotEmpty) ...[
                    ...
                    InkWell(
                      onTap: () => _showExtraServices(extraServices),
                      borderRadius: BorderRadius.circular(8),
                      child: Container(
                        width: double.infinity,
                        padding: const EdgeInsets.symmetric(
  ...
  void _showExtraServices(List<String> services) {
